...
double CompareHist(Mat hist1, Mat hist2, int method) { return cv::compareHist(*hist1, *hist2, method); }
...
struct Moment Moments(Mat src, bool binaryImage) {
    ...
    Moment mom = {m.m00,  m.m10,  m.m01,  m.m20,  m.m11,  m.m02,  m.m30,  m.m21,  m.m12,  m.m03,  m.mu20, m.mu11,
...
void HoughLinesPointSet(Mat points, Mat lines, int linesMax, int threshold, double minRho, double maxRho,
                        double rhoStep, double minTheta, double maxTheta, double thetaStep) {
    cv::HoughLinesPointSet(*points, *lines, linesMax, threshold, minRho, maxRho, rhoStep, minTheta, maxTheta,
                           thetaStep);
}

void Integral(Mat src, Mat sum, Mat sqsum, Mat tilted) { cv::integral(*src, *sum, *sqsum, *tilted); }

double Threshold(Mat src, Mat dst, double thresh, double maxvalue, int typ) {
    return cv::threshold(*src, *dst, thresh, maxvalue, typ);
}

void AdaptiveThreshold(Mat src, Mat dst, double maxValue, int adaptiveMethod, int thresholdType, int blockSize,
                       double c) {
...
void Watershed(Mat image, Mat markers) { cv::watershed(*image, *markers); }

void ApplyColorMap(Mat src, Mat dst, int colormap) { cv::applyColorMap(*src, *dst, colormap); }

void ApplyCustomColorMap(Mat src, Mat dst, Mat colormap) { cv::applyColorMap(*src, *dst, *colormap); }
...
CLAHE CLAHE_Create() { return new cv::Ptr<cv::CLAHE>(cv::createCLAHE()); }
...
void InvertAffineTransform(Mat src, Mat dst) { cv::invertAffineTransform(*src, *dst); }
...
void Mat_Accumulate(Mat src, Mat dst) { cv::accumulate(*src, *dst); }
void Mat_AccumulateWithMask(Mat src, Mat dst, Mat mask) { cv::accumulate(*src, *dst, *mask); }

void Mat_AccumulateSquare(Mat src, Mat dst) { cv::accumulateSquare(*src, *dst); }

void Mat_AccumulateSquareWithMask(Mat src, Mat dst, Mat mask) { cv::accumulateSquare(*src, *dst, *mask); }

void Mat_AccumulateProduct(Mat src1, Mat src2, Mat dst) { cv::accumulateProduct(*src1, *src2, *dst); }

void Mat_AccumulateProductWithMask(Mat src1, Mat src2, Mat dst, Mat mask) {
    cv::accumulateProduct(*src1, *src2, *dst, *mask);
}

void Mat_AccumulatedWeighted(Mat src, Mat dst, double alpha) { cv::accumulateWeighted(*src, *dst, alpha); }
